...
NSString *ExtractFileName(const char *filePath, BOOL extension, BOOL copy)
{
...
{
	return ExtractFileName(filePath, NO, copy);
}

NSString *ExtractFileNameWithExtension(const char *filePath, BOOL copy)
{
	return ExtractFileName(filePath, YES, copy);
}
...
- (NSString *)fileNameExt
{
	if (fileNameExt == nil)
	{
		fileNameExt = [ExtractFileNameWithExtension(file, NO) retain];
	}
